...
func loadRedisConf(cfg *RedisConfig) error {
	if err := config.Load("redis", cfg, func(v *viper.Viper) {
		...
		v.BindEnv("default.url", "APP_REDIS_URL")
		v.BindEnv("default.addr", "APP_REDIS_ADDR")
		v.BindEnv("default.password", "APP_REDIS_PASSWORD")
		v.BindEnv("default.DB", "APP_REDIS_DB")
	}); err != nil {
		return err
	}
